...
module Puppet::Parser::Functions
  ...
  newfunction(:helm_install_flags, :type => :rvalue) do |args|
    opts = args[0] || {}
    flags = []
    flags << 'install' if opts['ensure'].to_s == 'present'
    ...
    flags << "--key-file '#{opts['key_file']}'" if opts['key_file'].to_s != 'undef'
    flags << "--key-ring '#{opts['key_ring']}'" if opts['key_ring'].to_s != 'undef'
    ...
    flags << "--name '#{opts['release_name']}'" if opts['release_name'].to_s != 'undef'
    flags << "--name-template '#{opts['name_template']}'" if opts['name_template'].to_s != 'undef'
    flags << "--namespace '#{opts['namespace']}'" if opts['namespace'].to_s != 'undef'
